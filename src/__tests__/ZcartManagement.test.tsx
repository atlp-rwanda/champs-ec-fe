import userCartSlice, { handleCartCount, handleChangeCartQuantity, handleRemoveAllCart, handleRemoveItemInCart, handleUserAddCart } from "@/redux/slices/userCartSlice";
...
describe('userCartSlice', () => {
  ...
  it('should add a new product to an existing cart', async () => {
    ...
    const result2=await store.dispatch(handleUserAddCart({ productId: 'product2', quantity: 1 }));
    await handleUserAddCart({ productId: 'product2', quantity: 1 })(dispatch, () => initialState, undefined);
...
it('should update the quantity of a product in the cart successfully', async () => {
  ...
  expect(result.payload).toEqual({ data: 'success' });
  // Dispatch the handleRemoveItemInCart action
  const result2=await store.dispatch(handleChangeCartQuantity({ productId: 'product2', quantity: 1 }));
  await handleChangeCartQuantity({ productId: 'product2', quantity: 1 })(dispatch, () => initialState, undefined);
...
it('should remove a product from the cart successfully', async () => {
  ...
  localStorage.setItem('productCart', JSON.stringify({
    userId: 'user1',
    product: [{ product: 'product1', quantity: 2 }, { product: 'product2', quantity: 1 }]
  }));
  ...
  await handleRemoveItemInCart(productId)(dispatch, () => initialState, undefined);
  
  // Assertions
  expect(mockedRequest.put).toHaveBeenCalledWith('/carts', expectedItems);
  const actions = dispatch.mock.calls.map(call => call[0]);
  console.log("1111111111111111111111111111111111111111111111111111111111111111111111111111111",result)
  expect(result.payload).toEqual({ data: 'success' });
...
it('should update the cart and save to localStorage when result.cart is provided', () => {
  const actionPayload = {
    cart: { userId: 'user1', product: [{ productId: 'product1', quantity: 2 }] },
  };
  ...
  expect(localStorage.getItem('productCart')).toBe(JSON.stringify(actionPayload.cart));
});
