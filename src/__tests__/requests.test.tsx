...
import { getUsers, createUser, getProducts, createProduct } from '../utils/requests';
...
describe('API functions', () => {
  ...
  test('getUsers function', async () => {
    ...
    await expect(getUsers()).rejects.toThrow("Request failed with status code 401");
  });
  test('createUser function', async () => {
    axiosMock.post.mockRejectedValue({ response: { status: 404 } });
    await expect(createUser(userData)).rejects.toThrow("Request failed with status code 404");
  });
...
//   });
  ...
  test('createProduct function', async () => {
    axiosMock.post.mockRejectedValue({ response: { status: 401 } });
    await expect(createProduct({})).rejects.toThrow("Request failed with status code 401");
  });
});
