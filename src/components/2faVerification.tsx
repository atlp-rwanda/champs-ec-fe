...
import { handleOTPVerification,resendOTPCode } from '@/redux/slices/2faAuthenticationSlice';
...
const OtpVerify: React.FC<OtpVerifyInterface> = ({ isOpen }) => {
    ...
    const { loading, error,newOtp,isAuthenticated} = useAppSelector((state: RootState) => state.sellerOTP);
    ...
    const HandleInput = (index: number, element: React.ChangeEvent<null | HTMLInputElement>) => {
    ...
    return (
        <>
            {isOpen && <div className="fixed inset-0 bg-black bg-opacity-10 backdrop-blur-md mt-[50%] md:mt-[10%] z-40" data-testid="divSection">
                <dialog  open={isOpen} className='sm:w-[90%] md:w-[60%] w-[100%] rounded-[10px] backdrop-blur-lg fixed p-10'>
                    <img
                        src="/otp.png"
                        alt="OTP LOGO"
                        width={100}
                        ...
                        className='m-auto'
                    />
                    <h2 className='text-[30px] text-center m-2' data-testid="trigger-element">Two Step Verification</h2>
                    <div className='w-[90%] sm:w-[50%] md:w-[70%] m-auto mt-10'>
                        <p className='text-center sm:text-[20px] text-[13px]'>We have sent a verification code via your email please enter here</p>
                    </div>
                    ...
                    <div >
                        <h1 className='text-center text-warningRed text-[15px] my-4 sm:text-[20px]'>{error}</h1>
                    </div>
                    <div className='m-auto w-[90%] sm:w-[50%] flex flex-col md:flex-row gap-2'>
                        ...
                        <>
                         ...
                         <Link href="#" onClick={ResendCode} className="text-greenMain">Resend code</Link>
                        </>
