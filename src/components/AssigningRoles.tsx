...
import { useQuery, useMutation, RefetchOptions, QueryObserverResult, useQueryClient } from "@tanstack/react-query";
...
const AssigningRole: React.FC<AssignRoleInterface> = ({ id, currentRole, refetch }) => {
          ...
          <select ref={select} value={currentRole} className='capitalize focus:outline-none border border-blue-500 text-center text-black shadow outline-none text-[15px]  font-medium  border-2 focus:ring-black
            ...
            {Array.isArray(roles) &&
              roles.map((value, index) => (
                <option key={index} value={value.id} className='option capitalize h-[30px] bg-primaryGrey focus:bg-primaryGrey text-black px-3 py-2'>
