  ...
  const columns: ColumnDef<ordersTable>[] = [
    {
      ...
      cell: (val: any) => (
        <>
          <img
            src={val.getValue()}
            className="w-[60px] h-[40px] border-rounded-2"
          />
        </>
      ),
    },
    ...
    },
    {
      ...
      cell: (row: any) => (
        <>
          <span className="bold">
            {' '}
            {row.row.original.product.productPrice.toLocaleString()}
          </span>
          <span> {row.row.original.product.productCurrency}</span>
        </>
      ),
    },
    {
    ...
    <>
        ...
        <div className="bg-white min-w-[100%] min-h-[100%]  rounded-b-lg rounded-[20px] py-4 px-2">
          <>
            <div className="min-h-[500px] overflow-hidden mb-3">
              <div className="block min-h-[430px] px-0 py-0">
                <table className="text-black border-t-1 text-left w-[100%] border-spacing-4">
                  ...
                  <tbody>
                    {table.getRowModel().rows.map((row) => (
                      <tr
                        key={row.id}
                        className="border-b indent-0 text-[13px] py-3 my-6  w-full  h-[70px]"
                      >
            ...
            <div className="f-right">
              <DeleteButton
                name="Clear All"
                handle={() => handleOpenWishlistPopup(productId, 'wishlist')}
              />
            </div>
          </>
        </div>
      ...
      {isDeleteWishlistModalOpen && (
        <DeleteWishlistPopup
          ...
          refetch={refetch}
          handleClearWishlist={handleClearAll}
        />
      )}
    </>
  );
