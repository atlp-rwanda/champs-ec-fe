...
const UsersPageAdmin = () => {
      ...
      cell: (row: any) => (
        <div className='w-full flex justify-center gap-4 my-1.5 text-[15px]' >
          ...
          <DisableButton
            ...
            handle={() => handleOpenDeactivatePopup(row.row.original.id, row.row.original.isActive)}
  ...
  return (
    <>
      <div className='w-[100%]  flex w-[100%]'>
        <div className='w-[100%] sm:w-[70%] min-h-[600px] mx-auto mt-10 py-6  px-10 border border-primaryGrey shadow rounded-[10px]'>
          {!data ? (
            ...
            <>
              <h1 className='font-bold text-[20px] flex gap-8'>Users:<span className='text-primaryBlue hidden sm:block'> {Object.keys(usersData).length}</span></h1>
      ...
      {isDeactivateUserModalOpen &&
        <DeactivateUser isUserActive={isUserActive} id={userId} isOpen={isDeactivateUserModalOpen} handleClose={handleClosePopup} refetch={refetch}/>
      }
    </>
  );
};
