...
const ConfirmDelete: React.FC<DeleteProductInterface> = ({ productId, isOpen, toggleConfirmPopup, refetch}) => {
  ...
  return (
    <dialog ref={dialogRef} onClick={(e) => e.stopPropagation()} className='z-50 bg-white backdrop-filter backdrop-brightness-75 rounded shadow-lg w-[80%] py-6 px-4 sm:w-[30%] mx-auto'>
      ...
      <div className='w-[100%] m-auto sm:w-[50%] my-12 '>
        <h1 className='text-[2vh] text-center my-5 font-bold'> Delete Product</h1>
  
          <div className="my-5">
            <span className='text-[13px]'>Are you sure you want to delete this product?</span>
          </div>
        
        <DeleteButton name='Delete'  loading={mutation.isPending} btnColor='btn-red'  handle={handleDelete} />
      </div>
    </dialog >
  );
};
